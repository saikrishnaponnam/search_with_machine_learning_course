...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        if res and res["hits"]["hits"] and res["hits"]["total"]["value"] > 0:
            ...
            for result in results:
                ...
                feature_result = { feature.get("name"): feature.get("value", 0) for feature in features }
                feature_result["query_id"] = int(query_id)
                feature_result["doc_id"] = int(result['_id'])
                feature_result["sku"] = int(result['_source']['sku'][0])
                # feature_result["salePrice"] = float(result['_source']['salePrice'][0])
